<template>
  <view class="mescroll-uni-warp">
    ...
    <mescroll-top :class="isShowToTop ? isScroll? 'is-scroll':'no-scroll' :''"
                  v-model="isShowToTop"
...
export default {
  ...
  methods: {
    ...
    scroll (e) {
      ...
      uni.$emit('discoverBtn', false)
      this.mescroll.optUp.toTop.right = -80
      this.isScroll = true
      ...
      this.timer = setTimeout(() => {
        uni.$emit('discoverBtn', true)
        this.mescroll.optUp.toTop.right = 2
        this.isScroll = false
      }, 500)
    },
    // 点击空布局的按钮回调
